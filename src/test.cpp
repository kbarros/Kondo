...
namespace tibidy {
    ...
    double exactEnergy(arma::sp_cx_mat H, double kB_T, double mu) {
        auto eigs = arma::eig_gen(sparseToDense(H));
        double ret = 0.0;
        for (auto x : eigs) {
            ret += fermiEnergy(x.real(), kB_T, mu);
        }
        return ret;
    }
    ...
    void testExpansionCoeffs() {
        ...
        for (int Mq : Mqs) {
            ...
            auto c = expansionCoefficients(M, Mq, f, es);
    ...
    void testKPM1() {
        ...
        auto es = energyScale(H, extra, tolerance);
        auto engine = mkEngineCx(n, n);
        engine->setHamiltonian(H, es);
        allVectorsCx(engine->R);
        
        int M = 1000;
        int Mq = 4*M;
        auto g_c = expansionCoefficients(M, Mq, g, es);
        auto f_c = expansionCoefficients(M, Mq, f, es);
        
        double E1 = engine->trace(g_c);
        cout << "energy (v1) " << E1 << " expected 50.0004 for M=1000\n";
        
        auto gamma = momentTransform(engine->moments(M), Mq);
        double E2 = densityProduct(gamma, g, es);
    ...
    void testKPM2() {
        ...
        for (int i = 0; i < n; i++) {
            auto x = 1.0 + noise * randomNormalCx(rng);
            int j = (i-1+n)%n;
            mb.add(i, j, x);
            mb.add(j, i, conj(x));
        }
        ...
        auto g = std::bind(fermiEnergy, _1, kB_T, mu);
        auto f = std::bind(fermiDensity, _1, kB_T, mu);
        ...
        auto g_c = expansionCoefficients(M, Mq, g, es);
        auto f_c = expansionCoefficients(M, Mq, f, es);
        auto engine1 = mkEngineCx(n, n);
        auto engine2 = mkEngineCx(n, n/4);
        engine1->setHamiltonian(H, es);
        engine2->setHamiltonian(H, es);
        ...
        allVectorsCx(engine1->R);
        double E2 = engine1->trace(g_c);
        auto dE_dH_2 = engine1->deriv(f_c);
        
        uncorrelatedVectorsCx(rng, engine2->R);
        ...
        correlatedVectorsCx(grouping, rng, engine2->R);
